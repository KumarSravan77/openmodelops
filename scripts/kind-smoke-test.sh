#!/usr/bin/env bash
set -euo pipefail
curl --fail --silent --retry 12 --retry-delay 2 http://localhost:18004/health/ready >/dev/null
curl --fail --silent --retry 12 --retry-delay 2 http://localhost:18006/health/ready >/dev/null
